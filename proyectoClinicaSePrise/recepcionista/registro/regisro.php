...
<head>
    ...
    <?php
        ...
        $mensaje1 = '';
        if(isset($_GET['dniBuscar']) && $_GET['dniBuscar'] == ""){
            $consulta = "select distinct * from paciente";
            $resultado = mysqli_query($conexion,$consulta);
        }
        else if(isset($_GET['dniBuscar']) && $_GET['dniBuscar'] != ""){
        // Recupera y muestra el valor de la variable de sesión
            $dniBuscar = $_GET['dniBuscar'];

            $consulta = "select distinct * from paciente where numDocumentoPaciente = $dniBuscar";
            $resultado = mysqli_query($conexion,$consulta);
        }    
        else{
            $consulta = "select distinct * from paciente";
...
<body>
    ...
    <main class="modulos">
        ...
        <div class="titulo-insumo">
            <p class="titulo-insumo-text">Registrar Paciente</p>
        </div>    
        ...
        <form action="alta_paciente_sql.php" method="post" class="formulario-insumo">
            <div class="inputs">
                ...
                <input type="text" id="nroDoc" name="nroDoc" placeholder="Numero de Documento" requierd>
            </div>
            <div class="inputs">
        ...
        <form action="busar_paciente.php" method="post" class="formulario-insumo">
            <div class="inputs">
                <label for="nroDoc" class="label-input">Numero de Documento:</label>
                <input type="text" id="nroDoc" name="dniBuscar" placeholder="Ingrese documento a buscar">
            </div>
            <div class="botones">
                <button button type="submit" class="btn-search">Buscar</button>
            </div>
            <?php echo $mensaje1; ?>   
        </form>
        ...
        <div class="volver">
            <button class="btn-volver"><a href="http://localhost/proyectoClinicaSePrise/recepcionista/main-recepcionista.php">Volver</a></button>
        </div>
